...
export function CariPestisidaView() {
  ...
  const performSearch = (hamaVal: string, tanamanVal: string) => {
    const qHama = hamaVal.trim().toLowerCase();
    const qTanaman = tanamanVal.trim().toLowerCase();
    ...
    const scored = PESTISIDA_CATALOG.map(item => {
      ...
      if (qTanaman && (isMatch || !qHama)) {
        ...
        const directCropMatch = itemText.includes(qTanaman);
        const isGeneralCrop = itemText.includes('semua tanaman') || itemText.includes('berbagai tanaman') || itemText.includes('hortikultura');

        if (directCropMatch) {
          score += 10;
        } else if (isGeneralCrop) {
          score += 4;
        } else if (!qHama) {
          // If search was ONLY by crop and no direct match
          score += 1;
        }
      }
    ...
    if (filtered.length === 0 && (qHama || qTanaman)) {
      // Fallback search with broad words
      filtered = scored.filter(i => {
        const fullText = (i.nama + ' ' + i.bahanAktif + ' ' + i.sasaran.join(' ')).toLowerCase();
        return (qHama && fullText.includes(qHama)) || (qTanaman && fullText.includes(qTanaman));
      });
    }

    if (filtered.length === 0) {
      filtered = PESTISIDA_CATALOG.slice(0, 15).map(item => ({ ...item, score: 0, isMatch: true }));
    }
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setHasSearched(true);
      performSearch(hamaInput, tanamanInput);
      setIsSearching(false);

      let msg = 'Menampilkan seluruh database pestisida.';
      if (hamaInput && tanamanInput) {
        msg = `Hasil diperbarui! Ditemukan rekomendasi untuk Hama: ${hamaInput} & Tanaman: ${tanamanInput}.`;
      } else if (hamaInput) {
        msg = `Hasil diperbarui! Ditemukan rekomendasi untuk Hama: ${hamaInput}.`;
      } else if (tanamanInput) {
        msg = `Hasil diperbarui! Ditemukan rekomendasi untuk Tanaman: ${tanamanInput}.`;
      }
  ...
  // Helper untuk menentukan Rekomendasi Bahan Aktif & Prinsip 5 Tepat berdasarkan Hama & Tanaman Target
  const getRumus5Tepat = (hama: string, tanaman: string, item: PestisidaItem) => {
    const h = hama.toLowerCase();
    const t = tanaman.toLowerCase();
    ...
    let bahanAktifRekomendasi = item.bahanAktif;
    ...
    if (h.includes('ulat grayak') || h.includes('spodoptera')) {
      ...
      bahanAktifRekomendasi = 'Emamektin benzoat, Klorantraniliprol, Tetraniliprol, Spinotoram, atau Profenofos';
      ...
      bahanAktifRekomendasi = 'Abamektin, Imidakloprid, Spirotetramat, Tiametoksam, Fipronil, atau Dinotefuran';
      ...
      bahanAktifRekomendasi = 'Azoksistrobin, Difenokonazol, Propineb, Mankozeb, Tebukonazol, atau Tiofanat Metil';
      ...
      bahanAktifRekomendasi = 'Tembaga Oksisulfat, Streptomisin Sulfat, atau Kasugamisin';
      ...
      bahanAktifRekomendasi = 'Abamektin, Piridaben, atau Diafentiuron';
    ...
    return {
      tepatSasaran,
      tepatJenis: `${item.jenis} dengan bahan aktif (${item.bahanAktif}) [Rekomendasi standar: ${bahanAktifRekomendasi}]`,
        ...
        {feedbackToast && (
          <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[9999] max-w-lg w-[calc(100vw-2rem)] mx-auto pointer-events-auto animate-in slide-in-from-top-3 duration-200">
            <div className={`p-3.5 rounded-xl border-2 border-[#0A0A0A] shadow-[4px_4px_0px_0px_#0A0A0A] flex items-center justify-between gap-3 ${
              feedbackToast.type === 'success' ? 'bg-[#154734] text-white font-bold' : 'bg-[#FFFFFF] text-[#0A0A0A] font-bold'
            }`}>
          ...
          {results.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {results.map((item, idx) => {
                ...
                const isTopRec = idx === 0 && (item as any).score >= 10;
