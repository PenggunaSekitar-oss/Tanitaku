...
// Let's just find the last button
const searchStr = '<button onClick={() => navigate(\'log\')} className="neo-btn bg-surface-high hover:bg-surface text-on-surface text-[11px] min-h-[44px]">CATAT LOG</button>';
const index = content.indexOf(searchStr);

if (index !== -1) {
    const startStr = content.slice(0, index + searchStr.length);
    const endStr = `
            </div>
          </div>
        </div>
...
`;
    fs.writeFileSync('src/views/DashboardView.tsx', startStr + endStr);
}
