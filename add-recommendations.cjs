              ...
              return (
                <div key={t.id} className="neo-card-small p-4 bg-surface-high/30 flex flex-col gap-3">
                  <div className="flex justify-between items-center border-b border-outline pb-2">
                    <div>
                      <h3 className="font-display font-bold text-lg text-on-surface">{t.komoditas} <span className="text-on-surface-muted text-sm font-normal">({t.varietas})</span></h3>
                      <span className="text-xs font-mono text-on-surface-muted">{blok?.nama || 'Unknown'}</span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="font-display font-bold text-xl text-black bg-action px-2 py-0.5 rounded-[4px_2px_4px_2px] neo-border-thin shadow-[2px_2px_0px_0px_#000] inline-block">{hst} <span className="text-sm font-sans font-normal text-black/70">HST</span></span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                    ...
                    <div className="flex flex-col md:col-span-2">
                      ...
                      <span className="text-sm font-sans bg-action/10 text-on-surface p-2 rounded-sm border-l-2 border-action">{rekomendasi.tips}</span>
                    </div>
                  </div>
                </div>
              );
